...
static inline int craftax_clamp_index(int index, int size) {
    if (index < 0) {
        index += size;
    }
    if (index < 0) {
...
static inline int craftax_step_mini32(int a, int b) {
    return a < b ? a : b;
}

static inline int craftax_step_maxi32(int a, int b) {
    return a > b ? a : b;
}
...
static inline int craftax_step_get_max_health(const CraftaxState* state) {
    int max_health = 8 + state->player_strength;
    int initial_health = (int)ceilf(g_craftax_initial_health);
    return initial_health > max_health ? initial_health : max_health;
}

static inline int craftax_step_get_max_food(const CraftaxState* state) {
    return 7 + 2 * state->player_dexterity;
}

static inline int craftax_step_get_max_drink(const CraftaxState* state) {
    return 7 + 2 * state->player_dexterity;
}

static inline int craftax_step_get_max_energy(const CraftaxState* state) {
    return 7 + 2 * state->player_dexterity;
}

static inline int craftax_step_get_max_mana(const CraftaxState* state) {
...
static inline void craftax_step_direction(int action, int direction[2]) {
    direction[0] = 0;
    direction[1] = 0;
    int direction_index = craftax_clamp_index(action, 16);
...
static inline bool craftax_step_is_solid_block(int block) {
    switch (block) {
...
static inline void craftax_boss_logic(CraftaxState* state) {
    ...
    state->boss_timesteps_to_spawn_this_round -=
        (int)craftax_step_is_fighting_boss(state);
}
...
static inline void craftax_level_up_attributes(
    ...
    state->player_dexterity += (int)is_levelling_up_dex;
    state->player_strength += (int)is_levelling_up_str;
    state->player_intelligence += (int)is_levelling_up_int;
    state->player_xp -= (int)is_levelling_up;
}
...
static inline void craftax_update_player_intrinsics(
    ...
    int hungered_food = craftax_step_maxi32(
        state->player_food - (int)not_boss,
        0
    );
    int new_food = new_hunger > 25.0f ? hungered_food : state->player_food;
    ...
    int thirsted_drink = craftax_step_maxi32(
        state->player_drink - (int)not_boss,
        0
    );
    int new_drink = new_thirst > 20.0f ? thirsted_drink : state->player_drink;
    ...
    int new_energy = new_fatigue > 30.0f
        ? craftax_step_maxi32(state->player_energy - (int)not_boss, 0)
        : state->player_energy;
    ...
    int new_mana = new_recover_mana > 30.0f
        ? state->player_mana + 1
        : state->player_mana;
...
static inline void craftax_drink_potion(
    ...
    drinking_potion_index = (int)is_drinking_red_potion * 0
        + (1 - (int)is_drinking_red_potion) * drinking_potion_index;
    ...
    drinking_potion_index = (int)is_drinking_green_potion * 1
        + (1 - (int)is_drinking_green_potion) * drinking_potion_index;
    ...
    drinking_potion_index = (int)is_drinking_blue_potion * 2
        + (1 - (int)is_drinking_blue_potion) * drinking_potion_index;
    ...
    drinking_potion_index = (int)is_drinking_pink_potion * 3
        + (1 - (int)is_drinking_pink_potion) * drinking_potion_index;
    ...
    drinking_potion_index = (int)is_drinking_cyan_potion * 4
        + (1 - (int)is_drinking_cyan_potion) * drinking_potion_index;
    ...
    drinking_potion_index = (int)is_drinking_yellow_potion * 5
        + (1 - (int)is_drinking_yellow_potion) * drinking_potion_index;
    is_drinking_potion = is_drinking_potion || is_drinking_yellow_potion;

    int potion_index = craftax_clamp_index(drinking_potion_index, 6);
    int potion_effect_index = state->potion_mapping[potion_index];

    int delta_health = 0;
    delta_health += (int)is_drinking_potion * (int)(potion_effect_index == 0) * 8;
    delta_health += (int)is_drinking_potion * (int)(potion_effect_index == 1) * -3;

    int delta_mana = 0;
    delta_mana += (int)is_drinking_potion * (int)(potion_effect_index == 2) * 8;
    delta_mana += (int)is_drinking_potion * (int)(potion_effect_index == 3) * -3;

    int delta_energy = 0;
    delta_energy += (int)is_drinking_potion * (int)(potion_effect_index == 4) * 8;
    delta_energy += (int)is_drinking_potion * (int)(potion_effect_index == 5) * -3;
    ...
    state->inventory.potions[potion_index] =
        state->inventory.potions[potion_index] - (int)is_drinking_potion;
...
static inline void craftax_read_book(
    ...
    int learn_spell_achievement = spell_to_learn_index
    ...
    state->inventory.books -= (int)is_reading_book;
